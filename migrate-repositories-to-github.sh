...
do
  ...
  curl -u vijeths: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 https://api.github.com/orgs/$gh_org/repos -d "{\"name\": \"$gh_repo\", \"private\": $PRIVATE_FLAG, \"description\": \"$description\"}"
  echo

  echo "=== pushing $gh_org/$gh_repo to GitHub..."
  git push --mirror https://vijeths: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@github.com/$gh_org/$gh_repo.git
  echo

  if [ "$archive" = "yes" ]; then
    echo "=== archiving repository $gh_org/$gh_repo on GitHub..."
    curl -X PATCH -u vijeths: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 https://api.github.com/repos/$gh_org/$gh_repo -d "{\"archived\": true}"
  fi
